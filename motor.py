...
class MotorAnalise:
    ...
    # --- 1. STRESS TEST (Cenários de Risco) ---
    def calcular_stress_test(self, ticker, qtd, preco_atual):
        try:
            ...
            beta = 1.0
            try:
                # Tenta calcular Beta contra IBOV
    ...
    def analisar_sentimento_news(self, ticker):
        try:
            ...
            pos = ["lucro", "alta", "dividend", "compra", "recorde", "profit"]
            neg = ["prejuízo", "queda", "fraude", "corrupção", "divida", "loss"]
            for n in news[:5]:
    ...
    def detectar_padroes_graficos(self, h, l, c):
        ...
        try:
            n = 5
            # Importante: Variáveis renomeadas para evitar erro de sintaxe
            ...
            if len(topos)>=2:
                ...
                if (t2[0]-t1[0]>20) and abs(t1[1]-t2[1])/t1[1]<0.05:
    ...
    def analisar(self, hist, info, ticker, modo_crise=False):
        try:
            ...
            if p_justo > 0:
                if atual <= p_teto: sc_q += 30; motivos.append("Valuation: Top")
                elif atual <= p_justo: sc_q += 10; motivos.append("Valuation: OK")
            ...
            if rsi < 30: sc_c += 10; motivos.append("Oportunidade RSI")
